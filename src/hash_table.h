 ...
 *  - key_eq_check: A predicate function that checks for equivalency between two
 * keys.
 *  - value_eq_check: A predicate function that checks for equivalency between
 * two values.
 */
struct hash_table {
    ...
    ioopm_hash_function
        *hash_func; // Function that converts our union it into a size_t (hash)
...
static link_t **find_previous_pointer(const ioopm_hash_table_t *ht,
                                      link_t **first_link, const elem_t key);
static link_t *get_linked_list_head(const ioopm_hash_table_t *ht,
                                    const elem_t key);
static void initialize_hash_table(ioopm_hash_table_t **ht,
                                  bucket_t *bucket_array,
                                  const size_t bucket_count,
                                  ioopm_hash_function *hash_func,
                                  ioopm_eq_function *key_eq_check,
...
static int prepend_key(elem_t key,
                       elem_t *value_unusued __attribute__((unused)),
                       void *key_list);
static int prepend_value(elem_t key,
                         elem_t *value_unusued __attribute__((unused)),
                         void *key_list);
static int remove_key_from_table(elem_t key,
                                 elem_t *value_unusued __attribute__((unused)),
                                 void *ht);
static void get_value(link_t *link_ptr, void *return_value);
static bool apply_to_one(ioopm_hash_table_t *ht, const elem_t key,
                         iterate_function iterate_func, void *arg_iterate);
static void do_nothing(link_t *link_ignored __attribute__((unused)),
                       void *args_ignored __attribute__((unused)));
static void hash_table_resize(ioopm_hash_table_t **ht,
                              ioopm_hash_function *hash_func,
                              ioopm_eq_function *key_eq_check,
                              ioopm_eq_function *value_eq_check);

int ioopm_create_hash_table(ioopm_hash_table_t **return_ht, size_t bucket_count,
                            ioopm_hash_function *hash_func,
                            ioopm_eq_function *key_eq_check,
                            ioopm_eq_function *value_eq_check) {
    // Check presumptions
    ...
    *return_ht = malloc(sizeof(ioopm_hash_table_t));
    ...
    initialize_hash_table(return_ht, bucket_array, bucket_count, hash_func,
                          key_eq_check, value_eq_check);

    return SUCCESS;
}

int ioopm_hash_table_insert(ioopm_hash_table_t *ht, const elem_t key,
                            const elem_t value) {
    // Check presumptions
    // Checks if the argument pointer is valid, returns an error status
    if (ht == NULL) {
    ...
    if (((ht)->key_count / (double)(ht)->bucket_count) > 1.5) {
        hash_table_resize(&ht, ht->hash_func, ht->key_eq_check,
                          ht->value_eq_check);
    }
...
int ioopm_hash_table_lookup(bool *return_found, elem_t *return_value,
                            const ioopm_hash_table_t *ht, const elem_t key) {
    // Check presumptions
    ...
    // Update return_value and return_found to reflect whether the key is in the
    // hash table
    *return_found = (apply_to_one((ioopm_hash_table_t *)ht, key, get_value,
                                  (void *)return_value));

    return SUCCESS;
}

int ioopm_hash_table_keys(ioopm_list_t **return_key_list,
    ...
    ioopm_hash_table_apply_to_all((ioopm_hash_table_t *)ht, prepend_key,
                                  *return_key_list);

    return SUCCESS;
}

int ioopm_hash_table_values(ioopm_list_t **return_value_list,
                            const ioopm_hash_table_t *ht) {
    // Check presumptions
    // Checks if the argument pointers are valid, returns an error status
    if (ht == NULL) {
    ...
    int status =
        ioopm_linked_list_create(return_value_list, ht->value_eq_check);
    if (status != SUCCESS) {
        return status;
    }

    // Prepend each value to value_list
    status = ioopm_hash_table_apply_to_all((ioopm_hash_table_t *)ht,
                                           prepend_value, *return_value_list);
...
int ioopm_hash_table_remove(ioopm_hash_table_t *ht, const elem_t key) {
    ...
    link_t **prev =
        find_previous_pointer(ht, &(ht->buckets[get_index(ht, key)]), key);
...
int ioopm_hash_table_has_key(bool *return_found, const ioopm_hash_table_t *ht,
                             const elem_t key) {
    // Check presumptions
    // Checks if the argument pointers are valid, returns an error status
    if (ht == NULL) {
    ...
    // Check if key exists
    *return_found =
        apply_to_one((ioopm_hash_table_t *)ht, key, do_nothing, NULL);

    return SUCCESS;
}

int ioopm_hash_table_has_value(bool *return_found, const ioopm_hash_table_t *ht,
                               const elem_t value) {
    // Check presumptions
    // Checks if the argument pointers are valid, returns an error status
    if (ht == NULL) {
    ...
    // Check each bucket
    for (size_t bucket_index = 0; bucket_index < ht->bucket_count;
         bucket_index++) {
...
int ioopm_hash_table_is_empty(bool *return_empty,
                              const ioopm_hash_table_t *ht) {
    // Check presumptions
    // Checks if the argument pointers are valid, returns an error status
    if (ht == NULL) {
...
int ioopm_hash_table_all(bool *return_all, const ioopm_hash_table_t *ht,
                         ioopm_hash_table_predicate *pred, const void *arg) {
    // Check presumptions
    // Checks if the argument pointers are valid, returns an error status
    if (ht == NULL) {
        return INVALID_HASH_TABLE_POINTER;
    ...
    while (ioopm_iterator_has_next(&has_next_key, key_iter) == SUCCESS &&
           has_next_key &&
           ioopm_iterator_has_next(&has_next_value, value_iter) == SUCCESS &&
           has_next_value) {
...
int ioopm_hash_table_any(bool *return_any, const ioopm_hash_table_t *ht,
                         ioopm_hash_table_predicate *pred, const void *arg) {
    // Check presumptions
    // Checks if the argument pointers are valid, returns an error status
    if (ht == NULL) {
...
int ioopm_hash_table_apply_to_all(ioopm_hash_table_t *ht,
                                  ioopm_hash_table_apply_function *apply_fun,
                                  void *arg) {
    // Check presumptions
    // Checks if the argument pointer is valid, returns an error status
    if (ht == NULL) {
 ...
 * @param iterate_func A function pointer that is applied to a link specified by
 * the key.
 * @param extra Any extra arguments for the iterate function.
 * @param returns Returns true if the key was found, and by extension if the
 * iterate func was called. Otherwise, false.
 */
static bool apply_to_one(ioopm_hash_table_t *ht, elem_t key,
                         iterate_function iterate_func, void *extra) {
 ...
 */
static int prepend_value(elem_t key_unusued __attribute__((unused)),
                         elem_t *value, void *value_list) {
    int status = SUCCESS;
...
/**
 * @brief Gets a pointer to a linked list head that matches to a given keys hash
 * function.
...
static link_t *create_link(const elem_t key, const elem_t value, link_t *next) {
    ...
    *new_link = (link_t){
        .key_value_pair.key = key, .key_value_pair.value = value, .next = next};
    return new_link;
}
...
static size_t get_hash(const ioopm_hash_table_t *ht, const elem_t key) {
    if (ht->hash_func == NULL) {
        ...
        return (size_t)
            key.integer; // .integer reads the integer part of the elem_t
    } else {
 ...
 *        The hash function is h(k) = k mod t, where k is the key and t is the
 * number of possible indexes.
 * @param ht The hash table to get number of possible indexes and hash function
 * from.
 * @param key A unique key.
 * @return The index of where the unique key should be stored in the hash table
 * array.
 */
static size_t get_index(const ioopm_hash_table_t *ht, const elem_t key) {
...
static link_t **find_previous_pointer(const ioopm_hash_table_t *ht,
    ...
    // Key does not exist in this bucket, return pointer to pointer to head of
    // linked list
    return first_link;
}
 ...
 */
static void initialize_hash_table(ioopm_hash_table_t **ht,
                                  bucket_t *bucket_array, size_t bucket_count,
                                  ioopm_hash_function *hash_func,
                                  ioopm_eq_function *key_eq_check,
                                  ioopm_eq_function *value_eq_check) {
...
void hash_table_resize(ioopm_hash_table_t **ht, ioopm_hash_function *hash_func,
                       ioopm_eq_function *key_eq_check,
                       ioopm_eq_function *value_eq_check) {
    ioopm_hash_table_t *new_table = NULL;
    ioopm_create_hash_table(&new_table, (size_t)((*ht)->bucket_count * 2),
                            hash_func, key_eq_check, value_eq_check);
